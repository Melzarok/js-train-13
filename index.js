...
function searchParamsURL(url) {
  ...
  let paramsObj = urlObj.searchParams;
...
function checkURLParameters(url, params) {
  ...
  for (const item of params) {
    res[item] = urlObj.searchParams.has(item);
  }
...
function manipulateQuery(url, options) {
  ...
  if (options.has("append")) {
    let appendOptions = options.get("append");
  ...
  if (options.has("delete")) {
    let deleteOptions = options.get("delete");
...
function sortUrlParams(url) {
  ...
  let urlArray = Array.from(urlObj.searchParams.entries());

  urlArray.sort((a, b) => (a[0] > b[0] ? 1 : -1));
...
function getUrlKeys(url) {
  ...
  let keyArray = Array.from(urlObj.searchParams.keys());
